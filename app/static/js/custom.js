...
(function($){
	...
	function img_background(){
	    if ($(window).width() < 1200) {
	        $(".fon").css('background-image','url("")');
	        $(".fon1").css('background-image','url("")');
	    }
	    else {
	        if ($(window).width() > 1200){
	            $(".fon").css('background-image','url("../../static/images/fon.jpg")');
	        }
	    }
	...
	function sticky() {
		...
		var sticky_point = $('#page-wrapper').height()/$(window).height();

        /*if ($(window).scrollTop() > sticky_point)*/
        if ($(window).scrollTop() > 10){
            if (sticky_point>1.2){
                $("#header").addClass("header-sticky");
                $("#logo img").attr("src","../static/images/logo_small.png")
            }
        } else {
		...
		$("#contact-form").validate({
			...
			messages: {
				...
				email: {
					...
					email: "Пожалуйста укажите правильный email адрес!"
				},
				subject: {
			...
			submitHandler: function(form) {
				...
				$(form).ajaxSubmit({
					...
					success: function(msg) {
						...
						if (msg == 'OK') {
							/*result1 = '<div class="alert alert-success alert-dismissible" role="alert">Ваше сообщение успешно отправлено!</div>';*/
							result = '<div class="alert alert-success alert-dismissible fade in" role="alert"><button class="close" aria-label="Close" data-dismiss="alert" type="button"></button>Ваше сообщение успешно отправлено!</div>';
							$("#contact-form").clearForm();
						} else {
							result = '<div class="alert alert-danger alert-dismissible fade in" role="alert"><button class="close" aria-label="Close" data-dismiss="alert" type="button"></button>'+msg+'</div>';
						}
					...
					error: function() {
						...
						result = '<div class="alert alert-danger alert-dismissible fade in" role="alert"><button class="close" aria-label="Close" data-dismiss="alert" type="button"></button>Упссс, что-то пошло не так!</div>';
...
function footer(){
	    if (($(window).height()-$('#page-wrapper').height())>30) {
...
function wrapper_window(){
    if (($(window).height()-$('#page-wrapper').height())>30) {
        $('#page-wrapper').css('height',$(window).height()-30);

    }
    else{
        if (($(window).height()-$('#page-wrapper').height())<0){
            $('#page-wrapper').css('height','100%');
        }
    }
}
$("li.switch>a").bind("click", function(){
			$("li.switch.active").removeClass("active");
			var clickedURL = $( this).attr('href');
			$("div.display").hide();
			$("div.display").removeClass("display");
			$(clickedURL).addClass("display").show();
